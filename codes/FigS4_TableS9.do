...
******************************
	...
	reghdfe tall inter_c_t1 if year == 2020, absorb(i.dsp i.date) vce(cluster dsp)
		parmest , saving($results/figS4_tableS9/allage.dta, replace)
		outreg2 using $results/figS4_tableS9/tableS9.xml, dec(3) append bracket nocon nor2 noobs label addstat(Obs., e(N), Adjusted R-Square, e(r2_a),  # of DSP Counties, e(N_clust)) addtext(DSP Fixed Effect, "YES", Date Fixed Effect, "YES") 
	...
	foreach i of varlist tall cvd injury pneumonia{
		...
		parmest , saving($results/figS4_tableS9/age1_`i'w.dta, replace)
		outreg2 using $results/figS4_tableS9/tableS9.xml, dec(3) append bracket nocon nor2 noobs label addstat(Obs., e(N), Adjusted R-Square, e(r2_a),  # of DSP Counties, e(N_clust)) addtext(DSP Fixed Effect, "YES", Date Fixed Effect, "YES") 
			
		reghdfe `i'_agegroup2 inter_c_t2 if year == 2020, absorb(i.dsp i.date) vce(cluster dsp)
		parmest , saving($results/figS4_tableS9/age2_`i'w.dta, replace)
		outreg2 using $results/figS4_tableS9/tableS9.xml, dec(3) append bracket nocon nor2 noobs label addstat(Obs., e(N), Adjusted R-Square, e(r2_a),  # of DSP Counties, e(N_clust)) addtext(DSP Fixed Effect, "YES", Date Fixed Effect, "YES") 
			
		reghdfe `i'_agegroup3 inter_c_t3 if year == 2020, absorb(i.dsp i.date) vce(cluster dsp)
		parmest , saving($results/figS4_tableS9/age3_`i'w.dta, replace)
		outreg2 using $results/figS4_tableS9/tableS9.xml, dec(3) append bracket nocon nor2 noobs label addstat(Obs., e(N), Adjusted R-Square, e(r2_a),  # of DSP Counties, e(N_clust)) addtext(DSP Fixed Effect, "YES", Date Fixed Effect, "YES") 
...
* figure
		...
		* group
		...
		graph save age_log_tt, replace 
		
		
		graph combine age_ab_tt.gph age_log_tt.gph , ///
				graphregion(fcolor(white) lcolor(white)) imargin(small) col(2) iscale(0.75) fysize(150) fxsize(120) xsize(8) ysize(6)
				
				
			
		graph save "$figures/figS4/figS4.gph", replace 
		graph export "$figures/figS4.eps", replace
		graph export "$figures/figS4.png", replace
